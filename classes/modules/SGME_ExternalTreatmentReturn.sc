...
SGME_ExternalTreatmentReturn : SGME_Connectable  {
	...
	*addSynthDef {
		lag = SGME_Settings.get[\returnLag];
		SynthDef(\SGME_externalTreatmentReturn, {
			arg inputBus,
			outputBus,
			level,
			outVol;
	...
	createSynth {
		if(synth.isPlaying==false, {
			synth = Synth(\SGME_externalTreatmentReturn, [
				\inputBus, inputBus,
				\outputBus, outputBus,
	...
	convertLevel {|level|
		^level.linlin(0, 10, 0, settings[\returnLevelMax]);
	}
